...
function generateImage() {
    ...
    const query = `${input1}`;
    const query2 = `${input2}`;
    imageUrl = `https://api.agungny.my.id/api/pornhub?text1=${encodeURIComponent(query)}&text2=${encodeURIComponent(query2)}`;
...
function startCountdown() {
    ...
    let countdownTime = 3; // Atur hitungan mundur dalam detik
...
function downloadImage() {
    ...
    xhr.onload = function () {
        ...
        if (xhr.status === 200) {
            ...
            a.href = url;
            a.download = "PornHub By AG.png"; // Nama file saat di-download
